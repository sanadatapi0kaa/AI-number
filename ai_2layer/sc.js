...
const fff = ["weight1", "weight2", "weight3", "biases1", "biases2", "biases3"];
...
let gakusyuuper1 = 0.007;   //gakusyuu in - hide1
let gakusyuuper2 = 0.007;   //gakusyuu hide1- hide2
let gakusyuuper3 = 0.01;   //gakusyuu hide2 - out
let flatInput = 0;
let hidden1 = 0;
let hidden2 = 0;
let output = 0;
...
let weight1 = 0; //in - hide1 [16[784]]
let weight2 = 0; //hide1 - hide2 [16[784]]
let weight3 = 0; //hide2 - out [10[16]]
let biases1 = 0; //hide1
let biases2 = 0; //hide2
let biases3 = 0; //out

let sa1 = 0; //in - hide1 weight1 [16[784]]
let sa2 = 0; //hide1 - hide2 weight2 [16[784]]
let sa3 = 0; //hide2 - out weight3 [10[16]]
let sa4 = 0; //hide1 biases1 16
let sa5 = 0; //hide2 biases2 16
let sa6 = 0; //out biases3 10
let delta_hidden1 = 0;
let delta_hidden2 = 0;
...
if (input === 0 || input === 1) {
  weight1 = heInit(784, hidelayer1);
  fs.writeFileSync("weight1.json", JSON.stringify(weight1));
}

if (input === 0 || input === 2) {
  weight2 = heInit(hidelayer1, hidelayer2);
  fs.writeFileSync("weight2.json", JSON.stringify(weight2));
}

if (input === 0 || input === 3) {
  weight3 = heInit(hidelayer2, 10);
  fs.writeFileSync("weight3.json", JSON.stringify(weight3));
}

if (input === 0 || input === 4) {
  biases1 = Array.from({ length: hidelayer1 }, () => 0);
  fs.writeFileSync("biases1.json", JSON.stringify(biases1));
}

if (input === 0 || input === 5) {
  biases2 = Array.from({ length: hidelayer2 }, () => 0.01);
  fs.writeFileSync("biases2.json", JSON.stringify(biases2));
}

if (input === 0 || input === 6) {
  biases3 = Array.from({ length: 10 }, () => 0);
  fs.writeFileSync("biases3.json", JSON.stringify(biases3));
}
...
function forward(input) {
  ...
  hidden1 = Array(hidelayer1).fill(0);
  hidden2 = Array(hidelayer2).fill(0);
  flatInput = input.flat();

  for (let i = 0; i < hidelayer1; i++) {
    for (let j = 0; j < 784; j++) {
      hidden1[i] += (flatInput[j] * weight1[i][j]);
    }
    hidden1[i] += biases1[i];
    hidden1[i] = relu(hidden1[i]);
  }
  
  for (let i = 0; i < hidelayer2; i++) {
    for (let j = 0; j < hidelayer1; j++) {
      hidden2[i] += (hidden1[j] * weight2[i][j]);
    }
    hidden2[i] += biases2[i];
    hidden2[i] = leakeyrelu(hidden2[i]);
  }

  output = Array(10).fill(0);
  for (let i = 0; i < 10; i++) {
    for (let j = 0; j < hidelayer2; j++) {
      output[i] += (hidden2[j] * weight3[i][j]);
    }
    output[i] += biases3[i];
  }
  //output is big
...
function backpropagate(label) {
  ...
  sa1 = Array.from({ length: hidelayer1 }, () => Array(784).fill(0));
  sa2 = Array.from({ length: hidelayer2 }, () => Array(hidelayer1).fill(0));
  sa3 = Array.from({ length: 10 }, () => Array(hidelayer2).fill(0));
  sa4 = Array(hidelayer1).fill(0);
  sa5 = Array(hidelayer2).fill(0);
  sa6 = Array(10).fill(0);
  delta_hidden1 = Array(hidelayer1).fill(0);
  delta_hidden2 = Array(hidelayer2).fill(0);
  ...
  for (let i = 0; i < hidelayer2; i++) {
    for (let j = 0; j < 10; j++) {
      delta_hidden2[i] += cost[j] * weight3[j][i];
    }
  }
  
  //sa5
  for (let i = 0; i < hidelayer2; i++) {
    delta_hidden2[i] *= hidden2[i] > 0 ? 1 : 0;
    sa5[i] += delta_hidden2[i];
  }
  
  //sa2
  for (let i = 0; i < hidelayer2; i++) {
    for (let j = 0; j < hidelayer1; j++) {
      sa2[i][j] += delta_hidden2[i] * hidden1[j];
    }
  }
  ...
  for (let i = 0; i < hidelayer1; i++) {
    for (let j = 0; j < hidelayer2; j++) {
      delta_hidden1[i] += delta_hidden2[j] * weight2[j][i];
    }
  }

  //sa4
  for (let i = 0; i < hidelayer2; i++) {
    delta_hidden1[i] *= hidden1[i] > 0 ? 1 : 0;
    sa4[i] += delta_hidden1[i];
  }
  
  //sa1
  for (let i = 0; i < hidelayer1; i++) {
    for (let j = 0; j < 784; j++) {
      sa1[i][j] += delta_hidden1[i] * flatInput[j];
    }
  }
...
function kaisei(){
  ...
  for (let i = 0; i < 10; i++) {
    for (let j = 0; j < hidelayer2; j++) {
      weight3[i][j] -= sa3[i][j] * gakusyuuper3 * 2;
    }
    biases3[i] -= sa6[i] * gakusyuuper3 * 2;
  }

  weight3 = weight3.map((row) => row.map((v) => Math.max(-3, Math.min(v, 3))));
  biases3 = biases3.map((v) => Math.max(-1, Math.min(v ?? 0, 1)));

  for (let i = 0; i < hidelayer2; i++) {
    for (let j = 0; j < hidelayer1; j++) {
      weight2[i][j] -= sa2[i][j] * gakusyuuper2 * 2;
    }
    biases2[i] -= sa5[i] * gakusyuuper2 * 2;
  }

  weight2 = weight2.map((row) => row.map((v) => Math.max(-3, Math.min(v, 3))));
  biases2 = biases2.map((v) => Math.max(-1, Math.min(v ?? 0, 1)));

  for (let i = 0; i < hidelayer1; i++) {
    for (let j = 0; j < 784; j++) {
      weight1[i][j] -= sa1[i][j] * gakusyuuper1 * 2;
    }
    biases1[i] -= sa4[i] * gakusyuuper1 * 2;
  }
...
function douki(isfirst) {
  ...
  if (allExist) {
    ...
    weight3 = JSON.parse(fs.readFileSync("weight3.json", "utf8"));
    if (isfirst) {
    ...
    biases3 = JSON.parse(fs.readFileSync("biases3.json", "utf8"));
    if (isfirst) {
  ...
  if (fs.readFileSync("weight3.json", "utf8") == 0) resetnum(3);
  if (fs.readFileSync("biases1.json", "utf8") == 0) resetnum(4);
  if (fs.readFileSync("biases2.json", "utf8") == 0) resetnum(5);
  if (fs.readFileSync("biases3.json", "utf8") == 0) resetnum(6);
...
async function runTrainingLoop(imageFolderPath, auto) {
  ...
  //画像それぞれ
    ...
    //事後処理、console
      ...
      if (times % 10000 === 0) {
        gakusyuuper1 = gakusyuuper1 / 2;
        gakusyuuper1 = gakusyuuper1 / 2;
        console.log(`b, bnを0.001減らしました ${times}回 b= ${gakusyuuper1}`);
      }
    //log書き出し
...
function saveNetworkToFolder() {
  ...
  //a*20, a*25? 推奨
  let hennsuus = {weight1, weight2, weight3, biases1, biases2, biases3};
...
async function start() {
  ...
  if (answer === "c") {
    ...
    console.log(`今： ${gakusyuuper1}, ${gakusyuuper2}, ${gakusyuuper3}`);
    answer = readlineSync.question("値: ");
    if (answer !== "") {
      gakusyuuper1 = parseFloat(answer);
      gakusyuuper2 = parseFloat(answer);
      gakusyuuper3 = parseFloat(answer);
    }
    console.log(`バイアス：${gakusyuuper1} ${gakusyuuper2}, ${gakusyuuper3}`);
    start();
  }else if (answer === 's') {
    saveNetworkToFolder();
    start();
